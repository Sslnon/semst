...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        # add same id buckets for sampler
        self.buckets = {}
        for idx, e in enumerate(self.data_list):
            utt = e.get("utt_id")
            self.buckets.setdefault(utt, []).append(idx)
    ...
    def __getitem__(self, index):
        ...
        prompt_ids = self.tokenizer.encode(prompt)
        prompt_length = len(prompt_ids)   # 9

        # asr_text, _ = target.split("<|st|>", 1)
        # asr_length = len(self.tokenizer.encode(asr_text))

        # if source == "fleurs_en_us"
        # print(asr_text)
        # print(asr_length)
        # print("====================")
        # assert False
    ...
    def collator_pair(self, samples):
        batch = self.collator(samples)
        feats = batch["audio_mel"] if self.input_type=="mel" else batch["audio"]
        batch["view1"] = feats[0::2]   # 默认采样器保证顺序：英语在前
        batch["view2"] = feats[1::2]
        # 顺带返回源字段 / utt 方便 debug
        batch["utt_ids"]   = [s["utt_id"] for s in samples][0::2]
        batch["source_v1"] = [s["source"] for s in samples][0::2]
        batch["source_v2"] = [s["source"] for s in samples][1::2]
        return batch
