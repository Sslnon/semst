...
from slam_llm.utils.deepspeed_utils_cl import (
    train,
...
def main(kwargs: DictConfig):
    ...
    # names_in_opt = {id(p) for g in model_engine.optimizer.param_groups for p in g['params']}
    # for n,p in model.named_parameters():
    #     print(n, id(p) in names_in_opt)
    ...
    # for train_dl_kwargs in ["sampler", "batch_size", "drop_last", "shuffle"]:
    #     kwargs.pop(train_dl_kwargs, None)
    if train_config.batching_strategy== "pair":
        train_dl_kwargs = dict(train_dl_kwargs)
        for key in ["sampler", "batch_size", "drop_last", "shuffle"]:
            train_dl_kwargs.pop(key, None)
