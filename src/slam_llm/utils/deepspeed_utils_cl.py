...
def train(
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            model.train()
            # found_q = any(
            #     p is model.align_head.q_proj.weight
            #     for g in optimizer.param_groups for p in g['params']
            # )
            # print("[Check] q_proj.weight in optimizer?", found_q)   # 现在应是 True
            # found_q = any(
            # p is model.align_head.q_proj.weight
            ...
            # )
            # found_k = any(
            #     p is model.align_head.k_proj.weight
            #     for group in model.optimizer.param_groups
            #     for p in group['params']
            # )
            # print(f"[Check] q_proj.weight in optimizer? {found_q}")
            # print(f"[Check] k_proj.weight in optimizer? {found_k}")
            # assert False 
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                acc = rest[0] if rest[0] else -1
                loss_w = rest[1] if rest[1] else -1
                ot_layer = rest[2] if rest[2] else None
                ...
                ot_loss_val = loss_w.detach()
                model.ot_sampler.update_ucb(ot_layer, ot_loss_val.item())

                # if model.ema_loss[ot_layer] is None and ot_layer is not None:
                #     model.ema_loss[ot_layer] = ot_loss_val.item()
                # else:
                #     model.ema_loss[ot_layer] = alpha * model.ema_loss[ot_layer] + (1-alpha) * ot_loss_val.item()

                # if (step + 1) % 64==0:
                #     loss_tensor = torch.tensor(model.ema_loss)
                #     weights = torch.softmax(-(loss_tensor - loss_tensor.mean()) / tau, dim=-1)
                #     weights = torch.clamp(weights, 0.0, 0.7)   
                #     weights = weights / weights.sum()                
                #     model.ot_sampler.weights = weights.clone()


                loss = outputs.loss + 10 * loss_w
                # + 10 * loss_w
                # outputs.loss
                            ...
                            swanlab.log(
                                {
                                    ...
                                    "train_inner/st_loss": outputs.loss,
                                    "train_inner/cl_loss": loss_w,
                    ...
                    swanlab.log(
                        {
                            ...
                            # "train/train_sampler": model.ot_sampler.weights.tolist()[0]
                        }
                    )
            else:
                swanlab.log(
